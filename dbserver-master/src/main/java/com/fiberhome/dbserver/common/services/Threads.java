...
{
    protected static final Logger LOG = LoggerFactory.getLogger(Threads.class);
     ...
     * @param t thread to frob
     * @param name new name
     * @param handler A handler to set on the thread. Pass null if want to use
     *            default handler.
     * @return Returns the passed Thread <code>t</code>.
     */
    public static Thread setDaemonThreadRunning(final Thread t, final String name,
    ...
    /**
     * Sleeps for the given amount of time even if interrupted. Preserves the
     * interrupt status.
     * 
     ...
     * @return threadPoolExecutor the cachedThreadPool with a bounded number as the
     *         maximum thread size in the pool.
     */
    public static ThreadPoolExecutor getBoundedCachedThreadPool(int maxCachedThread, long timeout, TimeUnit unit,
        ThreadFactory threadFactory)
    {
        ThreadPoolExecutor boundedCachedThreadPool = new ThreadPoolExecutor(maxCachedThread, maxCachedThread, timeout,
    ...
    /**
     * Returns a {@link ThreadFactory} that names each created
     * thread uniquely, with a common prefix.
     *
    ...
    public static ThreadFactory getNamedThreadFactory(final String prefix)
    {
        ...
        return new ThreadFactory()
        {
            ...
            {
                final String name = prefix + "pool-" + poolNumber + "-thread-" + threadNumber.getAndIncrement();
    ...
    /**
     * Get a named {@link ThreadFactory} that just builds daemon threads and set
     * priority
     * 
     * @param prefix name prefix for all threads created from the factory
     ...
     * @return a thread factory that creates named, daemon threads
     */
    public static ThreadFactory newDaemonPriorityThreadFactory(final String prefix, final int priority)
    {
        final ThreadFactory namedFactory = getNamedThreadFactory(prefix);
        return new ThreadFactory()
        {
